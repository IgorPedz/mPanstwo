...
export default function FinalExamModal({ courseSlug, courseId, onClose, onPass }) {
  ...
  const questions = t(`courses.${courseSlug}.final_exam.questions`, { returnObjects: true });
  const ui = (key, opts) => t(`courses.final_exam.${key}`, opts);

  const total = Array.isArray(questions) ? questions.length : 0;
  const current = questions?.[step];

  const saveCourseCompletion = async (finalScore) => {
    if (finalScore < PASS_THRESHOLD) return;
    setSaving(true);
    try {
      await axios.post("http://localhost:5000/courses/complete", {
        userId: user?.id,
        courseId,
        score: finalScore,
      });
  ...
  const passed = isFinished && score >= PASS_THRESHOLD;
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {!total ? (
            ...
            >
              <p className="text-slate-400">Brak pytań egzaminacyjnych.</p>
            </motion.div>
          ) : !isFinished ? (
            <motion.div
              key={step}
            ...
            >
              <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-800/60 color-transition">
                <span className="px-2.5 py-1 text-[10px] font-black uppercase tracking-wider rounded-md bg-amber-500/10 text-amber-600 dark:text-amber-400">
                  {ui("title")}
                </span>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">
                  {ui("pass_info", { pass: PASS_THRESHOLD, total })}
                </p>
              </div>
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="space-y-3">
                  {current?.answers?.map((answer, i) => {
                    ...
                    if (selected !== null) {
                      if (isCorrect) {
                        styles = "bg-emerald-500/10 border-emerald-500 text-emerald-600 dark:text-emerald-400 font-bold";
                      } else if (isSelected) {
                        styles = "bg-red-500/10 border-red-500 text-red-600 dark:text-red-400 font-bold";
                      } else {
                        styles = "opacity-40 border-slate-200 dark:border-slate-800";
                      }
                    }
            ...
            >
              ...
              {passed ? (
                <button
                  ...
                  className="w-full p-4 rounded-xl font-black bg-amber-500 hover:bg-amber-400 text-white transition cursor-pointer disabled:opacity-60"
                >
                  {saving ? "Zapisywanie..." : `🎓 ${ui("back_to_courses")}`}
                </button>
              ) : (
                <button
                  onClick={handleRetry}
