const TYPE_LABEL = {
  STANDING:      { text: "Stała",         cls: "text-slate-400 dark:text-slate-500" },
  EXTRAORDINARY: { text: "Nadzwyczajna",  cls: "text-sky-600 dark:text-sky-400" },
  INVESTIGATIVE: { text: "Śledcza",       cls: "text-amber-600 dark:text-amber-400" },
};
...
export default function MPCommittees({ committees, loading }) {
  ...
  if (!committees.length) {
    return <p className="text-sm text-slate-400 dark:text-slate-500 color-transition">Brak danych o przynależności do komisji.</p>;
  }
          ...
          <div key={i} className="px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 color-transition space-y-1.5">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                {typeLabel && (
                  <p className={`text-[10px] mt-0.5 font-bold color-transition ${typeLabel.cls}`}>{typeLabel.text}</p>
                )}
              </div>
