...
import { VOTE_CFG, STAT_KEYS, getVote, normalizeVote, resolveVoteLabel } from "./mpProfileConstants";
import VotingDetailModal from "../Legislacja/VotingDetailModal";
    ...
    <>
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            <div className="flex flex-col items-start">
              <span className="text-xs font-black text-slate-900 dark:text-white color-transition">{t("institution.mp.sitting", { num: sitting })}</span>
            ...
            <div className="hidden sm:flex items-center gap-2 flex-wrap">
              {STAT_KEYS.map((k) => stats[k] > 0 && (
                <span key={k} className="flex items-center gap-1 text-[9px] font-bold text-slate-500 dark:text-slate-400">
                  ...
                  {t(VOTE_CFG[k].labelKey)} {stats[k]}
                </span>
              ))}
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 color-transition">{t("institution.mp.votesShort", { count: statsTotal })}</span>
            ...
            >
              <div className="border-t border-slate-100 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800 color-transition">
                {votes.map((v, i) => {
                  ...
                  const topic = v.topic ?? v.title ?? `${t("institution.mp.votings")} ${t("institution.mp.votingNr", { num: v.votingNumber })}`;
                  return (
                    <button
                      key={i}
                      onClick={() => setSelected(v)}
                      className="w-full flex items-start justify-between gap-3 px-5 py-3
                        bg-white dark:bg-slate-900 text-left cursor-pointer
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <p className="text-[11px] font-medium leading-snug
                          ...
                          transition-colors color-transition line-clamp-2">
                          {topic}
                        </p>
                        <p className="text-[9px] text-slate-400 dark:text-slate-500 mt-0.5 color-transition">
                          {t("institution.mp.votingNr", { num: v.votingNumber })}
                        </p>
                      </div>
                      <span className={`shrink-0 flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-black ${cfg.bg} ${cfg.text}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${cfg.dot}`} />
                        {resolveVoteLabel(cfg, t)}
                      </span>
                    </button>
                  );
                })}
      ...
      {selected && (
        <VotingDetailModal
          ...
          votNum={selected.votingNumber}
          summary={{
            sitting:   selected.sitting,
            votingNum: selected.votingNumber,
            date:      selected.date,
