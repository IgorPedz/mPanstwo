    ...
    <div
      ...
      className={`group relative h-35 w-35 flex flex-col items-center justify-center
        ...
        ${isAdded
          ...
          : `bg-gray-50 dark:bg-gray-700 hover:scale-[1.05] hover:shadow-lg ${isSelected ? "ring-4 ring-blue-400 ring-offset-2 ring-offset-gray-50 dark:ring-offset-gray-800" : ""}`
        }`}
    >
      {IconComponent && (
        <IconComponent
          className={`h-12 w-12 mb-3 transition-transform duration-300 ${isAdded ? "text-gray-400 dark:text-gray-500" : `${tile.iconColor} dark:${tile.iconColor}`}`}
        />
      )}
