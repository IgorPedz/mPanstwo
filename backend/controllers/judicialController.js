...
/* ── Trybunał Konstytucyjny — wyroki ze strony głównej ───────────────────── */
async function getTKNews(req, res) {
  try {
    const { data: html } = await axios.get("https://trybunal.gov.pl", { headers: HEADERS, timeout: 12_000 });

    // Pattern: <a href="/postepowanie-i-orzeczenia/..." title="Tytuł">...<div class="widget-subtitle">Sygnatura, DD M YYYY</div>
    const items = [];
    ...
    while ((m = re.exec(html)) !== null && items.length < 8) {
      ...
      items.push({
        ...
        lead:   sig,
        date,
        url:    `https://trybunal.gov.pl${path}`,
        image:  null,
      });
    }
...
/* ── Naczelny Sąd Administracyjny — aktualności ──────────────────────────── */
async function getNSANews(req, res) {
  try {
...
/* ── Krajowa Rada Sądownictwa — RSS feed ─────────────────────────────────── */
async function getKRSNews(req, res) {
  try {
    const { data: xml } = await axios.get(
      "https://www.krs.pl/pl/aktualnosci.feed?type=rss",
    ...
    while ((m = re.exec(xml)) !== null && items.length < 8) {
      ...
      items.push({
        title:  stripHtml(title),
        lead:   stripHtml(desc).slice(0, 150) || null,
        date:   rfcToDisplay(pubDate),
        url:    link.trim(),
...
/* ── Sąd Najwyższy — scraper strony głównej ──────────────────────────────── */
async function getSNNews(req, res) {
  try {
    ...
    // Każda wiadomość: <h3><a href="...ItemSID=NNN...">Tytuł</a></h3>
    // + opcjonalnie paragraf z tekstem i datą w pobliżu
    const seen    = new Set();
    const results = [];
    ...
    while ((m = linkRe.exec(c)) !== null && results.length < 8) {
      ...
      // Kontekst 1500 znaków po linku — szukamy daty i lead
...
module.exports = { getJudicialNews };
